...
const CertChain = {
  ...
  registerIssuer: function (hash) {
    let self = this
    console.log('Reached registerIssuer')
  ...
  FindRecipient: function (hash) {
    let self = this
    return new Promise((resolve, reject) => {
      self.instance.FindRecipient.call(
        hash,
        {from: window.web3.eth.accounts[0]}
      ).then(tx => {
        resolve(tx)
  ...
  checkValidCertificate: function (hash) {
    let self = this
    return new Promise((resolve, reject) => {
      self.instance.checkValidCertificate.call(
        hash,
        {from: window.web3.eth.accounts[0]}
      ).then(tx => {
        resolve(tx)
  ...
  checkValidCombo: function (hash1, hash2) {
    let self = this
    return new Promise((resolve, reject) => {
      self.instance.checkValidCombo.call(
        hash1,
        hash2,
        {from: window.web3.eth.accounts[0]}
      ).then(tx => {
